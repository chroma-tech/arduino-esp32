...
2) rtc_xtal_freq_t rtc_xtal_freq = rtc_clk_xtal_freq_get(); switch
(rtc_xtal_freq) { case RTC_XTAL_FREQ_40M: // Recommended
        // 50 MHz = 40MHz * (4 + 6) / (2 * (2 + 2) = 50.000
        // sdm0 = 0, sdm1 = 0, sdm2 = 6, o_div = 2
        rtc_clk_apll_enable(true, 0, 0, 6, 2);
        break;
    case RTC_XTAL_FREQ_26M:
        // 50 MHz = 26MHz * (4 + 15 + 118 / 256 + 39/65536) / ((3 + 2) * 2)
= 49.999992
        // sdm0 = 39, sdm1 = 118, sdm2 = 15, o_div = 3
        rtc_clk_apll_enable(true, 39, 118, 15, 3);
        break;
    case RTC_XTAL_FREQ_24M:
        // 50 MHz = 24MHz * (4 + 12 + 255 / 256 + 255/65536) / ((2 + 2) * 2)
= 49.499977
...
#if CONFIG_ETH_RMII_CLK_INPUT
    if(eth_clock_mode != ETH_CLOCK_GPIO0_IN && eth_clock_mode !=
ETH_CLOCK_GPIO0_OUT){ pinMode(0, INPUT);
    }
#endif
    ...
    if(eth_clock_mode == ETH_CLOCK_GPIO0_IN){
        ...
        //gpio_hal_iomux_func_sel(PERIPHS_IO_MUX_GPIO0_U,
...
FUNC_GPIO0_CLK_OUT1);
            ...
            //gpio_hal_iomux_func_sel(PERIPHS_IO_MUX_GPIO16_U,
FUNC_GPIO16_EMAC_CLK_OUT);
            ...
            //gpio_hal_iomux_func_sel(PERIPHS_IO_MUX_GPIO17_U,
...
bool ETHClass::begin(uint8_t phy_addr, int power, int mdc, int mdio,
                     eth_phy_type_t type, eth_clock_mode_t clock_mode,
  ...
  if (type == ETH_PHY_DM9051) {
    ...
    mac_config.clock_config.rmii.clock_mode =
        (eth_clock_mode) ? EMAC_CLK_OUT : EMAC_CLK_EXT_IN;
    mac_config.clock_config.rmii.clock_gpio =
        (1 == eth_clock_mode)   ? EMAC_APPL_CLK_OUT_GPIO
        : (2 == eth_clock_mode) ? EMAC_CLK_OUT_GPIO
...
#endif
#if CONFIG_ETH_SPI_ETHERNET_DM9051
    // eth_dm9051_config_t dm_9051_config = ETH_DM9051_DEFAULT_CONFIG();
  }
#endif
  ...
  if (esp_netif_attach(eth_netif, esp_eth_new_netif_glue(eth_handle)) !=
      ESP_OK) {
  ...
  // FIX ME -- adresses issue
  // https://github.com/espressif/arduino-esp32/issues/5733
  delay(50);
...
bool ETHClass::begin(spi_device_handle_t spi, int8_t phy_addr, int power,
                     int int_pin, eth_phy_type_t type,
                     bool use_mac_from_efuse) {
  ...
  // force gpio driver to install isr
  attachInterrupt(
      int_pin, []() {}, 0);
...
#if CONFIG_ETH_SPI_ETHERNET_DM9051
  if (type == ETH_PHY_DM9051) {
    eth_dm9051_config_t dm9051_config = ETH_DM9051_DEFAULT_CONFIG(spi);
    dm9051_config.int_gpio_num = int_pin;
    eth_mac = esp_eth_mac_new_dm9051(&dm9051_config, &mac_config);
    eth_phy = esp_eth_phy_new_dm9051(&phy_config);
  }
#endif
